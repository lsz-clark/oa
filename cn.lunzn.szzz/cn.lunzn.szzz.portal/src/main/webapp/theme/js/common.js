...
var __lang = "zh-cn";
// 企业微信登录地址
var qywxLoginLocation = "https://open.work.weixin.qq.com/wwopen/sso/qrConnect?appid=wwc38d087779e08e90&agentid=1000006&redirect_uri=http%3a%2f%2foa.lunzn.com%3a6060%2fszzz%2fstaff%2floginsso&state=web_login@gyoss9";
//var qywxLoginLocation = rootPath+"/portal/common/login.html";
...
$("head").append('<link rel="stylesheet" href="'+rootPath+'/theme/css/common.css??v=20180328">');
...
$("head").append('<script type="text/javascript" src="'+rootPath+'/theme/js/inputValidate.js??v=20180328"></script>');
$("head").append('<script type="text/javascript" src="'+rootPath+'/theme/js/constant.js??v=20180328"></script>');
...
function showRequestLoadding() {
    $("body").append('<div id="dv_bg_loading" class="dv_bg_loading_css" style="background-color:#000000;opacity:0.3;"><img src="'+rootPath+'/theme/images/w_loading.gif"></div>');
}
		...
		if(response.code == "6011000"){
			$.modalTips({
    			...
    			buttons: [{text:$.i18n.prop("confirm"), callfunc:function(){
    				...
    				window.location.href= qywxLoginLocation;
    			}}]
    		});
		}else{
			...
			if(parseResult(response) && response.total > 0){
				...
				$.each(response.result, function(i){
					...
					tableJQ.find("thead th[mkey]").each(function(){
						var isVisible = true;
						var customizeKey = "";
						if($(this).attr("mcustomize")){
							customizeKey = $(this).attr("mcustomize");
						}
						if($(this).attr("mdisplay")){
							isVisible = $(this).attr("mdisplay");
						}
						...
						if(isVisible == true){
							if(customizeKey && tableInfo.hasOwnProperty(customizeKey)){
								tds.push('<td>'+ tableInfo[customizeKey](response.result[i]) +'</td>');
							}else{
								tds.push('<td>'+ text +'</td>');
							}
						}
                            ...
                            if(customizeKey && tableInfo.hasOwnProperty(customizeKey)){
                            	tds.push('<td style="display:none;">'+ tableInfo[customizeKey](response.result[i]) +'</td>');
							}else{
								tds.push('<td style="display:none;">'+ text +'</td>');
							}
						}
				...
				tableJQ.find("tbody tr").bind("click", function(){
					...
					if($(this).find("td:first").find(".choose-click").get(0)){
						var $chooseElem = $(this).find("td:first").find(".choose-click");
						if($chooseElem.attr("class").indexOf("glyphicon-unchecked") > -1){
							$(".choose-click").removeClass("glyphicon-unchecked glyphicon-ok-circle").addClass("glyphicon-unchecked").attr("mchecked","");
							$chooseElem.removeClass("glyphicon-unchecked").addClass("glyphicon-ok-circle").attr("mchecked","checked");
						}else{
							$(".choose-click").removeClass("glyphicon-unchecked glyphicon-ok-circle").addClass("glyphicon-unchecked").attr("mchecked","");
						}
					}
