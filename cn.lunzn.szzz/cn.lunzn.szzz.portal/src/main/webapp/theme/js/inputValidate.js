...
function AKCheckout() {
			...
			for ( var i = 0; i < checkouts.length; i = i + 1) {
				if(checkouts[i].id == key || checkouts[i].name == key || checkouts[i].className == key){
					checkouts[i].getJQueryElement.each(function() {
						...
						$.AK.addInformation1($(this), true, "", false);
					});
	...
	this.removeAll = function(){
		if(checkouts){
			for ( var i = 0; i < checkouts.length; i = i + 1) {
				...
				checkouts.splice(0, checkouts.length);
			}
		}
	};
	...
	// 获取第一个错误的元素
	this.getErrFirstDom = function(stepFlag) {
		var firstDom = null;
		// 循环所有需要添加校验的元素，并给所有添加onkeyup或是onchange事件
		for ( var i = 0; i < checkouts.length; i = i + 1) {
			// id：绑定为单绑定，以name或是class绑定的为多绑定
...
$.AK.bindCheckout = function(element, checkout, stepFlag) {
	...
	if(stepFlag != "onstep"){
		// 提交元素（一般指触发按钮）绑定校验
		return $.AK.initCheckout(element, checkout, val, false);
	}else{
		return true;
	}
};
...
$.AK.initCheckout = function(element, checkout, val, hideFlag) {
	...
	// 如果有错误宿主且元素不可见则换成它显示错误信息
	if (null != checkout.showElement && $("#" + checkout.showElement)[0]) {
	...
	if ($.AK.isNotEmpty(checkout.defaultValue) && $.AK.isNotEmpty(val)
		...
		$.AK.addInformation1(element, true, "", hideFlag);
		return true;
	}
	...
	if (!checkout.isRequired) {
		...
		if ($.AK.isEmpty(val)) {
			$.AK.addInformation1(element, false, checkout.emptyMsg, hideFlag);
			return false;
		}
...
$.AK.invalidateFunc = function(element, checkout, val, hideFlag) {
	...
	if (null == checkout.getRole) {
		...
		if (!checkout.checkExpr(val)) {
			$.AK.addInformation1(element, false, checkout.invalidMsg, hideFlag);
			return false;
		}
		$.AK.addInformation1(element, true, "", hideFlag);
		return true;
...
$.AK.addInformation = function(element, isCorrect, msg, hideFlag) {
	...
	if (isCorrect) {
		...
		// 给输入框加上错误对话框
		element_parent.append("<div class='ak_checkout_talk_default'><div class='ak_checkout_talk_tipicon'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + msg
				+ "</div></div>");
		
		var talkHeight = element_parent.find(".ak_checkout_talk_default").innerHeight();
 ...
 * @param hideFlag 错误提示框是否隐藏
 */
$.AK.addInformation1 = function(element, isCorrect, msg, hideFlag) {
	// 元素的父节点
	...
	element_parent.children(".ak_checkout_talk_default1").remove();
	...
	if (isCorrect) {
		...
		// 给输入框加上错误对话框
		element_parent.append("<div class='ak_checkout_talk_default1'><i class='ak_checkout_talk_tipicon1'></i><span style='margin-left: 3px;'>"+ msg +"</span></div>");

		if (hideFlag) {
			element_parent.children(".ak_checkout_talk_default1").hide();
		}
	}
};
